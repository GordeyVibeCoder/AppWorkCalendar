...
fun EarningsScreen(
    ...
    Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        OutlinedButton(onClick = { showPicker = true }) {
            ...
            Text("Период: ${range.first} — ${range.second}", modifier = Modifier.padding(start = 8.dp))
        }

        EarningsChart(values = grouped.values.toList())
        Text("Итоговая выручка: %.2f ₽".format(total))
    }
    ...
    if (showPicker) {
        ...
        DatePickerDialog(
            ...
            confirmButton = {
                ...
                }) { Text(if (firstClick == null) "Выбрать начало" else "Выбрать конец") }
            },
            dismissButton = { TextButton(onClick = { showPicker = false }) { Text("Закрыть") } }
        ) {
            Column {
                DatePicker(state = pickerState)
                if (firstClick != null) {
                    Row(modifier = Modifier.fillMaxWidth().padding(8.dp)) {
                        Text("Начало: $firstClick")
                    }
                }
            }
        }
